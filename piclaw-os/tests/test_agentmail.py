...
from piclaw.tools.agentmail import (
...
async def test_agentmail_create_inbox(mock_agentmail_config, monkeypatch):
    ...
    mock_client.inboxes.create = AsyncMock(
        return_value=MagicMock(email_address="agent@agentmail.to", inbox_id="ib_123")
    )
    ...
    result = await agentmail_create_inbox(
        mock_agentmail_config, display_name="My Agent", username="my-agent"
    )

    assert "agent@agentmail.to" in result
    assert "ib_123" in result
    mock_client.inboxes.create.assert_called_once_with(
        display_name="My Agent", username="my-agent"
    )
...
async def test_agentmail_list_messages(mock_agentmail_config, monkeypatch):
    ...
    mock_client.inboxes.messages.list = AsyncMock(
        return_value=MagicMock(messages=[mock_msg])
    )
