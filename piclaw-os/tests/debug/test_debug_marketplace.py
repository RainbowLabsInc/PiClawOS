...
async def main():
    ...
    try:
        ...
        fail(f"Import fehlgeschlagen: {e}"); return
    ...
    try:
        ...
        for msg in [
            ...
            if r:
                q, plz, rad = r.get('query',''), r.get('location',''), r.get('radius_km','')
    ...
    if SEEN_FILE.exists():
        ...
        info(f"Gespeicherte IDs: {len(d.get('seen', []))} | Aktualisiert: {d.get('updated','?')}")
    else:
