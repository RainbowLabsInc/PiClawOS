...
class TestPiInfo:
    ...
    def test_simulated_on_no_vcgencmd(self):
        ...
        with patch("piclaw.hardware.pi_info._vcgencmd", return_value=None):
            result = asyncio.run(
                __import__(
                    "piclaw.hardware.pi_info", fromlist=["read_pi_info"]
                ).read_pi_info()
            )
...
class TestI2CScanFallback:
    def test_scan_no_hardware(self):
        ...
        with patch.object(i2c_mod, "_scan_smbus2", side_effect=ImportError):
            with patch.object(
                i2c_mod, "_scan_i2cdetect", side_effect=Exception("no tools")
            ):
    ...
    def test_format_report_unknown(self):
        ...
        dev = I2CDevice(
            0x11, bus=1, name="Unknown", desc="?", category="unknown", known=False
        )
        result = I2CScanResult(bus=1, devices=[dev])
        report = format_scan_report([result])
        assert "?" in report
...
class TestSensorRegistry:
    ...
    def _make_sensor(self, name="test_dht", typ="DHT22"):
        ...
        return SensorDef(
            name=name, type=typ, description="Test sensor", config={"pin": 4}
        )
...
class TestSensorReading:
    ...
    def test_ds18b20_no_hardware(self):
        ...
        assert (
            "1-Wire" in reading.error or "w1" in reading.error.lower() or reading.error
        )
