...
class Agent:
    ...
    def _detect_marketplace_intent(self, text: str) -> dict | None:
        ...
        # Städtenamen erkennen (Österreich + Deutschland) immer extrahieren für Query-Cleanup
        city_match = _RE_CITIES.search(text_clean)
        extracted_city = _CITY_MAP[city_match.group(1).lower()] if city_match else None

        if not location and extracted_city:
