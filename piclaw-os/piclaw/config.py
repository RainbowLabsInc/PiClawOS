...
class ShellConfig:
    ...
    allowlist:   list = field(default_factory=lambda: [
        "ls","cat","echo","pwd","d","free","uname","date","whoami",
        "git","python3","pip","pip3","curl","wget",
        "systemctl","journalctl","apt","apt-get",
        "nano","grep","find","ps","top","htop",
        "ip","nmcli","iwconfig","ifconfig","ping","ss","netstat",
        "gpio","raspi-config","vcgencmd",
        "reboot","shutdown","powerof",
    ])
    blocklist:   list = field(default_factory=lambda: [
...
class ServicesConfig:
    managed: list = field(default_factory=lambda: [
        "piclaw-agent", "piclaw-api",
        "ollama", "homeassistant",
        "ssh", "avahi-daemon",
    ])
...
def _load_section(cls, raw: dict, key: str):
    return cls(**{k: v for k, v in raw[key].items()
                  if k in cls.__dataclass_fields__}) if key in raw else cls()
