...
# Lock file created by agent.py when InstallerAgent is active
INSTALLER_LOCK_FILE = CONFIG_DIR / "ipc" / "piclaw_installer.lock"
INSTALLER_HANG_TIMEOUT = 900  # 15 min → stale lock = probable hang
...
class Watchdog:
    ...
    def _check_installer_hang(self) -> list[WatchdogAlert]:
        """Alert if installer lock file is stale — installer likely hung."""
        alerts = []
        if not INSTALLER_LOCK_FILE.exists():
            return alerts
        try:
            age = time.time() - INSTALLER_LOCK_FILE.stat().st_mtime
            if age > INSTALLER_HANG_TIMEOUT:
                alerts.append(WatchdogAlert(
                    severity=AlertSeverity.CRITICAL,
                    category="security",
                    message="Installer Hang Detected",
                    detail=(
                        f"Lock file {INSTALLER_LOCK_FILE.name} is {int(age / 60)} min old. "
                        "InstallerAgent may have hung. Manual cleanup: "
                        f"rm {INSTALLER_LOCK_FILE}"
                    ),
                    hostname=self._hostname,
                ))
        except Exception as e:
            log.error("Installer hang check: %s", e)
        return alerts
