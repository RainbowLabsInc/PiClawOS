...
def _setup_api_logging() -> None:
    ...
    if not any(
        ...
        and getattr(h, "baseFilename", "").endswith("api.log")
...
async def lifespan(app: FastAPI):
    ...
    # ── Home Assistant VOR Agent starten ──────────────────────────
    # WICHTIG: ha_mod.start() MUSS vor Agent(_cfg) aufgerufen werden!
    # Agent.__init__() → _build_tools() → ha_mod.get_client()
    # Wenn HA danach gestartet wird, ist get_client() = None → keine HA-Tools
    try:
        from piclaw.tools import homeassistant as _ha_mod
        _ha_client = await _ha_mod.start()
        if _ha_client:
            log.info("API: Home Assistant verbunden")
        else:
...
async def memory_search(
    q: str, collection: str = "all", mode: str = "query", _: str = Depends(require_auth)
):
    ...
    try:
        while True:
            ...
            try:
                ...
                await _manager.send(
                    websocket, {"type": "error", "text": "Ungültige Nachricht"}
                )
                continue
...
async def wizard_save_config(body: dict, _: str = Depends(require_auth)):
    ...
    _apply(
        section.get("name"),
        lambda: cfg.agent_name,
        lambda v: setattr(cfg, "agent_name", v),
        "agent.name",
    )
    _apply(
        section.get("log_level"),
        lambda: cfg.log_level,
        lambda v: setattr(cfg, "log_level", v),
        "agent.log_level",
    )
    ...
    _apply(
        section.get("backend"),
        lambda: cfg.llm.backend,
        lambda v: setattr(cfg.llm, "backend", v),
        "llm.backend",
    )
    _apply(
        section.get("model"),
        lambda: cfg.llm.model,
        lambda v: setattr(cfg.llm, "model", v),
        "llm.model",
    )
    _apply(
        section.get("api_key"),
        lambda: cfg.llm.api_key,
        lambda v: setattr(cfg.llm, "api_key", v),
        "llm.api_key",
    )
    _apply(
        section.get("base_url"),
        lambda: cfg.llm.base_url,
        lambda v: setattr(cfg.llm, "base_url", v),
        "llm.base_url",
    )
    ...
    _apply(
        section.get("token"),
        lambda: cfg.telegram.token,
        lambda v: setattr(cfg.telegram, "token", v),
        "telegram.token",
    )
    _apply(
        section.get("chat_id"),
        lambda: cfg.telegram.chat_id,
        lambda v: setattr(cfg.telegram, "chat_id", v),
        "telegram.chat_id",
    )
    ...
    _apply(
        section.get("token"),
        lambda: cfg.discord.token,
        lambda v: setattr(cfg.discord, "token", v),
        "discord.token",
    )
    ...
    _apply(
        section.get("channel"),
        lambda: cfg.updater.channel,
        lambda v: setattr(cfg.updater, "channel", v),
        "updater.channel",
    )
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                return {
                    "ok": False,
                    "error": data.get("description", "Unbekannter Fehler"),
                }
    except Exception as e:
        return {"ok": False, "error": str(e)[:200]}
