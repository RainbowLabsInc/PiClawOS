...
if [[ -d "$SRC_DIR" ]]; then
    ...
    # Doppelte Verschachtelung bereinigen: piclaw-src hat install.sh etc.
    # die nicht in /opt/piclaw landen sollen
    rm -f "$INSTALL_DIR/install.sh"
    # Sicherstellen dass kein doppeltes piclaw/piclaw/ entsteht
    if [[ -d "$INSTALL_DIR/piclaw/piclaw" ]]; then
        warn "Doppelte Verzeichnisstruktur erkannt – bereinige..."
        rm -rf "$INSTALL_DIR/piclaw/piclaw"
    fi
...
systemctl enable piclaw-api piclaw-agent piclaw-watchdog 2>/dev/null
ok "Services registriert: piclaw-api, piclaw-agent, piclaw-watchdog"
