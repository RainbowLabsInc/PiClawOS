...
class _StripHTML(HTMLParser):
    ...
    def handle_data(self, data):
        if not self._skip and data.strip(): self._parts.append(data.strip())
    def text(self): return " ".join(self._parts)[:8000]

async def http_fetch(url, method="GET", body=None, headers=None):
    hdrs = {"User-Agent": "PiClaw/0.2 (Raspberry Pi)"}
    if headers: hdrs.update(headers)
    try:
        async with aiohttp.ClientSession(headers=hdrs,
