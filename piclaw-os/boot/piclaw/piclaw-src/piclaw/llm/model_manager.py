...
MODELS = {
    ...
    "phi3-mini-q4": {
        ...
        "size_gb": 2.2,
        "desc": "Gut fuer komplexe Aufgaben, ~30-90s Antwortzeit",
    },
    "tinyllama-q4": {
...
async def download_model(model_id: str = DEFAULT_MODEL_ID) -> str:
    ...
    print(f"\n\U0001f4e5 Downloading {info['name']} ({info['size_gb']:.1f} GB)...")
    print(f"  \u2192 {dest}\n")
...
def list_models() -> str:
    lines = ["Verfuegbare lokale Modelle:\n"]
    for mid, info in MODELS.items():
        ...
        lines.append(
            ...
            f"  {info['name']}\n"
            f"  {info['desc']}\n"
            f"  Groesse: ~{info['size_gb']:.1f} GB | {status}\n"
        )
    return "\n".join(lines)
