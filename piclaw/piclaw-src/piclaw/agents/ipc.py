...
def update_job_status(job_id: str, status: str, result: str = "",
                      error: str = ""):
    init_jobs_db()
...
def write_job_result(job_id: str, pages: int, summary: str,
                     raw: str, found: str = ""):
    init_jobs_db()
    ...
    with _conn(JOBS_DB) as con:
        ...
        """, (rid, job_id, datetime.now().isoformat(),
              pages, summary, raw[:50000], found))
...
def _row_to_job(row) -> CrawlJob:
    ...
    try:
        ...
        d["urls"] = []
    return CrawlJob(**{k: v for k, v in d.items()
                       if k in CrawlJob.__dataclass_fields__})
...
def init_watchdog_db(force: bool = False) -> None:
    ...
    with _conn(WATCHDOG_DB) as con:
        ...
        con.execute("CREATE INDEX IF NOT EXISTS idx_alerts_severity ON alerts(severity, ts)")
        ...
        con.execute("CREATE INDEX IF NOT EXISTS idx_integrity_path  ON integrity_log(filepath, ts)")
...
def write_alert(alert: WatchdogAlert):
    ...
    with _conn(WATCHDOG_DB) as con:
        ...
        """, (alert.id, alert.severity, alert.category, alert.message,
              alert.detail, alert.hostname, alert.ts,
              int(alert.sent), int(alert.read)))
...
def write_report(report: WatchdogReport):
    ...
    with _conn(WATCHDOG_DB) as con:
        ...
        """, (report.id, report.ts, report.period, report.summary,
              report.alerts_new, report.alerts_crit,
              int(report.system_ok), int(report.sent)))


def write_integrity_entry(filepath: str, sha256: str,
                          changed: bool, detail: str = ""):
    init_watchdog_db()
    with _conn(WATCHDOG_DB) as con:
        con.execute("""
            INSERT INTO integrity_log VALUES (?,?,?,?,?,?)
        """, (str(uuid.uuid4())[:12], datetime.now().isoformat(),
              filepath, sha256, int(changed), detail))
