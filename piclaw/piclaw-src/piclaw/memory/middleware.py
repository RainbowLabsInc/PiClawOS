...
from piclaw.llm.base import Message, ToolDefinition, LLMBackend, LLMResponse
from piclaw.memory.store import write_fact, write_daily_note, save_session
...
class MemoryMiddleware:
    ...
    async def enrich(self, messages: list[Message]) -> list[Message]:
        ...
        log.debug("Injecting %s memory results (%s chars)", len(good), len(memory_block))
    ...
    async def after_turn(self, user_text: str, assistant_text: str):
        ...
        """
        self._turn_count += 1
        self._session_msgs.append({"role": "user",      "content": user_text,      "ts": datetime.now().isoformat()})
        self._session_msgs.append({"role": "assistant", "content": assistant_text, "ts": datetime.now().isoformat()})
        ...
        try:
            ...
            for fact in facts:
                if isinstance(fact, dict) and "content" in fact:
                    ...
                    log.info("Memory extracted: [%s] %.60s", fact.get('category'), fact['content'])
            ...
            if facts:
                ...
                log.debug("Memory extracted %d facts – QMD update deferred to cron", len(facts))
