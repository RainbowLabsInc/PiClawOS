...
## 2024-05-03 - Avoid N+1 queries when fetching latest metrics by using SQLite bare columns
**Learning:** Using a loop to run individual queries to get the latest metric point for each metric name causes a severe N+1 query problem, taking O(n) queries. Using SQLite's bare column aggregation pattern (`SELECT MAX(ts) as ts, name, value, unit, tags FROM metrics GROUP BY name`) reduces this to a single O(1) query. In tests with 100 metrics and 50,000 points, this improved performance by ~11x (from ~36s to ~3s for 100 iterations).
**Action:** Always prefer bulk queries using `MAX()` and `GROUP BY` in SQLite to efficiently retrieve the latest row for each distinct group, avoiding N+1 loops in API endpoints.
