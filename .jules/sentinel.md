...
## 2025-04-19 - [CRITICAL] Fix plaintext password exposure in nmcli
**Vulnerability:** The Wi-Fi password was being passed as a command-line argument (`nmcli dev wifi connect SSID password PASS`), which exposes the plaintext secret to any local user via process lists (e.g. `ps aux`).
**Learning:** Process arguments are globally readable on many POSIX systems. When integrating with CLI tools that require secrets, relying on command line parameters poses a significant information disclosure risk.
**Prevention:** Always check if external tools support reading secrets via standard input (e.g., using `--ask` for `nmcli`). Use the subprocess `communicate(input=...)` method to securely pipe the secret to the process's standard input rather than passing it as an argument.
