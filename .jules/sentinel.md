...
## 2024-05-24 - Validate Shell Bypass in Allowlist Checks
**Vulnerability:** Command injection bypass via chaining characters (`&`, `\n`, `\r`) in `piclaw-os/piclaw/tools/shell.py`. The `_is_allowed` function used `shlex.split(command)[0]` to evaluate if the first command is within the allowlist, but missed crucial chaining operators. This allowed commands like `ls \n cat /etc/passwd` to pass the allowlist check because `shlex` parsed `ls` as the first token while the shell executed the subsequent injected commands.
**Learning:** Checking only the first token of a shell command using `shlex` is insufficient if the full string is later evaluated by a shell (e.g., `create_subprocess_shell`). You must comprehensively block all shell command separators, control characters, and background operators prior to validation.
**Prevention:** Always maintain a strict and comprehensive blocklist of shell metacharacters (e.g., `&`, `\n`, `\r`, `;`, `|`, `&&`, `||`) when processing untrusted input for shell execution, or completely avoid shell-based execution (`create_subprocess_exec`) in favor of parameterized command lists.

## 2024-05-20 - Command Injection in CLI Utilities
**Vulnerability:** Critical command injection vulnerability in `cmd_service` and `cmd_soul` inside `piclaw-os/piclaw/cli.py` via `os.system()` with unsanitized user inputs (`action` and `editor`).
**Learning:** `os.system()` combined with f-strings is a classic vector for command injection. Environment variables like `$EDITOR` should never be trusted or executed via a shell since an attacker can set `EDITOR="nano; rm -rf /"`.
**Prevention:** Always use `subprocess.run` or `subprocess.call` with argument lists (without `shell=True`) instead of `os.system()`. For inputs that naturally contain multiple arguments (like an editor command), use `shlex.split()` and validate that no shell metacharacters (`&`, `|`, `;`, `\n`) are present in the raw input before parsing.
